...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] a)
    {
        ...
        int numPetals = 6; // Number of petals in the flower
        FlowerTurtle[] petals = new FlowerTurtle[numPetals];

        for (int i = 0; i < numPetals; i++) {
            petals[i] = new FlowerTurtle();
        }

        double angleBetweenPetals = 360.0 / numPetals;
        ...
        for (int i = 0; i < numPetals; i++) {
            ...
            petals[i].speed(5);
            if(i%2==0){
               petals[i].penColor("red"); 
            }else{
                petals[i].penColor("pink");
            }
            
            petals[i].up();
            petals[i].setPosition(i,i); 
            petals[i].left(initialAngle);
            petals[i].down();
            initialAngle += angleBetweenPetals;
        }

        // Create and start threads for each petal
        Thread[] threads = new Thread[numPetals];
        for (int i = 0; i < numPetals; i++) {
            final int index = i;
